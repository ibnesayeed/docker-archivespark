...
LIB_PATH="$PROG_HOME"/../lib

eval exec "$SPARK_HOME"/bin/spark-submit \
  ${SPARK_OPTS} \
  --driver-class-path $PROG_HOME/lib/${KERNEL_ASSEMBLY}:$(echo "$LIB_PATH"/*.jar | tr ' ' ':') \
  --jars $PROG_HOME/lib/${KERNEL_ASSEMBLY},$(echo "$LIB_PATH"/*.jar | tr ' ' ',') \
  --class org.apache.toree.Main "${TOREE_ASSEMBLY}" "${TOREE_OPTS}" "$@"
